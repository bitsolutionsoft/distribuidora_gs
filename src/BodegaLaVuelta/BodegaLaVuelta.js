...
function BodegaLaVuelta(props)  {
  ...
  const [ubicacion, setubicacion] = useState("Bodega La Vuelta");
...
const tarea = () => {
  const socket=io(host,connectionOptions);
     ...
     socket.on("BodegaLaVuelta",function(data){
        console.log(data)
        consultarDespacho()
        notificar()
...
async function consultarProducto() {
  let dat = await DataProducto.consultarxUbicacion("Bodega La Vuelta");
  if (dat !== null) {
