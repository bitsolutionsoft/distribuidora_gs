...
async function numero_orden() {
    ...
    async function consultarProducto() {
        ...
        if (dat !== null) {
          ...
          //  console.log(dat.res);
          //setdatosOriginal(dat.res);
          let datosfiltrado =  returnFiltrado(dat.res);
          setdatosDefault(datosfiltrado)
            //setencontrado(datosfiltrado);
            setdatos(datosfiltrado);
          
        }
    }
    const verTodo = (params) => {
        if(todo){
            settodo(false)
            setdatos(returnFiltrado(datosOriginal))
            setencontrado(returnFiltrado(datosOriginal))
        }else{
            settodo(true)
            setencontrado(datosOriginal)
    ...
    const returnFiltrado = (params) => {
        
          
        let filt=params.filter(function(item){
          return item.ubicacion===("Tienda La Septima") || item.ubicacion===("Bodega La Septima") ;   
...
res=datos.map((item)=>{
if(item.idproducto === data.idproducto){
   item.cant_yarda=(Number(item.cant_yarda)+Number(data.yardatotal));
   item.cant_rollo=Math.trunc(Number(item.cant_yarda)/Number(item.yardaporrollo));
...
let fact={
    ...
    if(ingresado !== null){
        if(ingresado.message==="Success"){
            setIdFactura(ingresado.res[0].idfactura);
            await Promise.all(   
    ...
    returnTipoBodega(datosv,"BodegaLaSeptima",idFactura)
...
const ModificarPrecio=(data, nuevoPrecio)=>{
    datosv.map((item) =>{
        if(item.idproducto === data.idproducto){
            ...
            item.total=Number(item.yardatotal)*Number(item.precio_yarda);
...
<div className="h-contain d-flex flex-wrap-reverse">     
        <div className="col d-flex ">
             <div className="col">   
                <h6 className="modal-title ">Punto de Venta Tienda La Septima, Textiles G&S</h6>
              </div>
              <div className="row">
              <div className="col">  
        ...
        <h6 className="text-black">Lista de producto Disponible</h6>
            <div className="col">
            ...
            <div className="col"> 
               <div className="form-check form-check-inline">
                  <input className="form-check-input"  type="radio" name="exampleRadios" id="exampleRadios1"   value="" onChange={(e)=>BusquedaTodo(e.target.value)} />
                  <label className="form-check-label" htmlFor="exampleRadios1">Todos</label>
                </div> 
               <div className="form-check form-check-inline">
                  <input className="form-check-input" type="radio" name="exampleRadios" id="exampleRadios2" value="Bodega La Septima"  onClick={(e) => Busqueda2(e.target.value)}/>
                  <label className="form-check-label" htmlFor="exampleRadios2">Bodega Setptima Av.</label>
               </div>
                    ...
                    <table className="table-item">
                        ...
                        <tbody >
                            {datos ? datos.map((item,index) => (
                                <tr key={index} onClick={()=>{AgregarProducto(item,1,"Nuevo")}}>
