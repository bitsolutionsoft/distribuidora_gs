...
const tarea = () => {
  console.log(("conecta a web socket"));
  const socket=io(host,connectionOptions);

      console.log(socket)
     socket.on("Bodega4",function(data){
...
async function consultarProducto() {
  let dat = await DataProducto.consultarxUbicacion("Bodega 4");
  if (dat !== null) {
  console.log("🚀 ~ file: App.js ~ line 24 ~ consultarProducto ~ dat", dat);
    console.table(dat.res);
...
<option value={item.idproveedor}  key={index}> {item.nombre}</option>
     ))
     :
     null
}
